...
export default {
  ...
  methods: {
      ...
      try {
        const response = await axios.get(
          process.env.VUE_APP_API_URL +
            `api/users/search-worldnews?query=${this.query}`
        );
    ...
    async deleteWorldnews(id) {
      try {
        console.log(id);
        await axios.delete(
          process.env.VUE_APP_API_URL + `api/users/world-remove/${id}`,
          {
            headers: {
    ...
    async sortPopular() {
      try {
        const response = await axios.get(
          process.env.VUE_APP_API_URL + "api/worldnews/popular/sort"
        );
        this.worldnewsData = response.data;
      } catch (error) {
        console.error("Error");
